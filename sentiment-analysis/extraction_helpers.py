...
def extract_quotes_speaker_keywords(chunk, speaker, keywords):
    ...
    for keyword in keywords:
        ...
        dict_speaker_keywords[keyword] = chunk.loc[
            (chunk["quotation"].str.contains(keyword, na=False)) & (chunk["speaker"] == speaker)]
        ...
        with pd.read_json(data, lines=True, compression=compression, chunksize=chunksize) as df_reader:
            ...
            if speakers and keywords:
                ...
                for speaker in speakers:
                    ...
                    for keyword in keywords:
                        dict_speakers_keywords[speaker][keyword] = pd.DataFrame(
                ...
                for chunk in df_reader:
                    for speaker in speakers:
                        ...
                        for keyword in keywords:
                            dict_speakers_keywords[speaker][keyword] = pd.concat(
                                [dict_speakers_keywords[speaker][keyword], quotes[keyword]], ignore_index=True)
            ...
            # Speaker is given
                ...
                for speaker in speakers:
                    dict_speakers[speaker] = pd.DataFrame({'quoteID': '', 'quotation': '', 'speaker': '', \
                                                           'qids': '', 'date': '', 'probas': '', 'numOccurences': '',
                                                           'phase': ''}, index=[0])
